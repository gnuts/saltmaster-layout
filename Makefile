...
FORMULAS = conntrack-formula heartbeat-formula network-debian-formula quagga-formula users-formula
...
install: clean update-doc 
	...
	rsync -rv --exclude '*.rst' formulas $(INST_USRSHAREDIR)/
	rsync -rv saltsite-templates $(INST_USRSHAREDIR)/
	chown -R root:root $(INST_USRSHAREDIR)
	chmod -R u=rwX,go=rX $(INST_USRSHAREDIR)
